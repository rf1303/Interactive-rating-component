document.addEventListener('DOMContentLoaded', function () {
    ...
    ratingNumber.forEach((inputRadio, index) => {
        ...
        inputRadio.addEventListener('keydown', function(e) {
            ...
            const ir = (index + 1) % ratingNumber.length;
            const il = (index - 1 + ratingNumber.length) % ratingNumber.length;
            console.log('ir - il 🆕 ', ir, '⚡', il);

            if (e.key === 'Tab' && !e.shiftkey) {
                console.log('TAB');
                if (index < ratingNumber.length - 1) {
                    e.preventDefault();
                ...
                e.preventDefault();
                const indexRight = (index + 1) % ratingNumber.length;
                ...
                e.preventDefault();
                const indexLeft = (index - 1 + ratingNumber.length) % ratingNumber.length;
